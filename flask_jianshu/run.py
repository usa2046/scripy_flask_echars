...
def jianshu_timeline():
    ...
    all_month_data = user.all_tags_data_pd(time_period='month')
    all_day_data = user.all_tags_data_pd(time_period='day')
    all_hour_data = user.all_tags_data_pd(time_period='hour')
    all_week_data = user.all_tags_data_pd(time_period='week')
    ...
    for each in args[:5]:
        if baseinfo[each+'_num']>100:
            week_hour[each] = user.tag_week_hour_data_pd(each)
        else:
            week_hour[each] = []
