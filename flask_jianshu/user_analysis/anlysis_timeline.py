...
class AnalysisUser():
    ...
    def __init__(self, slug,*args):
        ...
        dfLst = []
        for dy in config.TIMELINE_TYPES:
            df = pd.DataFrame(user_data[dy])
            dfLst.append(df)
    ...
    def extract_month_data_pd(self):
        dti = pd.to_datetime(list(self.df.time))
        ts = pd.Series([1] * len(dti), index=dti)
        print(ts)
        tsDay = ts.resample("1M").sum()
        print(tsDay[tsDay > 0])
        lstDay = [x.strftime('%Y-%m') for x in tsDay[tsDay > 0].index]
        lst_freqency = list(tsDay[tsDay > 0].values)

        month_data = {'month_line': lstDay, 'month_freqency': list(map(int,lst_freqency))}
        return month_data

    def extract_day_data_pd(self):
        dti = pd.to_datetime(list(self.df.time))
        ts = pd.Series([1] * len(dti), index=dti)
        print(ts)
        ...
        lst_freqency = list(tsDay[tsDay > 0].values)
        day_data = {'day_line': lstDay, 'day_freqency': list(map(int,lst_freqency))}

        return day_data

    def extract_hour_data_pd(self):
        dti = pd.to_datetime(self.df.time).to_frame()
        print(dti)

        tsDay =dti.groupby(dti['time'].map(lambda x: x.hour)).count()
        print(tsDay)


        lst_freqency = [int(item[0]) for item in tsDay[tsDay > 0].values]
        # lstDay = [x.strftime('%Y-%m-%d') for x in tsDay[tsDay > 0].index]
        lstDay = [str(x).rjust(2,'0') + ':00' for x in tsDay[tsDay > 0].index]
        hour_data = {'hour_line': lstDay, 'hour_freqency': lst_freqency}
        #
        return hour_data

    def extract_week_data_pd(self):
        dti = pd.to_datetime(self.df.time).to_frame()
        print(dti)

        tsDay =dti.groupby(dti['time'].map(lambda x: x.dayofweek)).count()
        print(tsDay)
        ...
        week_day_dict = {0: '周一', 1: '周二', 2: '周三', 3: '周四',
                         4: '周五', 5: '周六', 6: '周日', }
        lstDay = [week_day_dict[x] for x in tsDay[tsDay > 0].index]
        week_data = {'week_line': lstDay,
                     'week_freqency': lst_freqency}
        return week_data

    def tag_week_hour_data_pd(self, args):
        df = pd.DataFrame(self.user_data[args])
        print(df)
        dti = pd.to_datetime(df.time).to_frame()
        print(dti)

        groupp = dti.groupby([dti['time'].map(lambda x: x.dayofweek).rename('dow'),
        ...
        tag_week_hour_data = []
        max_freq = 0
        for name, g in groupp:
            freq = tsDay.loc[name].values[0]
            if freq > max_freq:
                max_freq = freq
            print(tsDay.loc[name].values[0])
            each = [int(name[0]), int(name[1]), int(freq)]
            tag_week_hour_data.append(each)

        return [max_freq, tag_week_hour_data]


    def all_tags_data_pd(self,time_period):
        '''
        ...
        if time_period == 'month':
            all_tags_data = self.extract_month_data_pd()
        elif time_period == 'day':
            all_tags_data = self.extract_day_data_pd()
        elif time_period == 'hour':
            all_tags_data = self.extract_hour_data_pd()
        elif time_period == 'week':
            all_tags_data = self.extract_week_data_pd()
        else:
            all_tags_data = None
        # print(all_tags_data)
...
def extract_month_data(time_list):
    ...
    if time_list:
        ...
        return {'month_line': [], 'month_freqency': []}
